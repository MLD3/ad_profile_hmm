...
kGrid               = rowvec(repmat([12 14], 100, 1));

[kOpt, phiOpt]      = ndgrid(kGrid, 1);
nHmmExp             = numel(kOpt);

hmmModels           = cell(size(kOpt));
loglik              = zeros(nSeries, nHmmExp);
...
parfor exp=1:nHmmExp
    ...
    initModel       = getInitModel(traj, nStates, 'discrete', ...
        'method', 'score-perturbed', 'nDiscreteValues', nDiscreteValues, ...
        'score', score, 'nExpansions', 0, 'dx', dx, ...
        'sortModel', false, 'yl', yl, 'yu', yu, ...
        'leftToRight', false, 'nPerturbGroups', 5);
%     initFn          = @(~, X, r)getInitModel(cellfun(@transpose, ...
%         X, 'uniformoutput', false), nStates, 'discrete', 'method', ...
%         'score-perturbed', 'nDiscreteValues', nDiscreteValues, ...
...
%         'yl', yl, 'yu', yu, 'leftToRight', false, 'nPerturbGroups', 5);
    ...
    model               = hmmFitEm(ser, initModel.nstates, 'discrete', ...
        'model', initModel, 'initFn', [], 'verbose', false, ...
        'nRandomRestarts', 1);
